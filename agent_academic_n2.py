...
def call_ollama(prompt, is_final=False):
    ...
    if is_final:
        system_prompt = "あなたは学術的な総括を行う研究者です。ここまでの議論を踏まえ、最終的な結論と成果を200字程度でまとめてください。"
    else:
        system_prompt = "あなたは放送大学の学術的背景に精通した研究者です。以下の共有ノートを読み、学術的な視点や理論的な裏付けを加えて200字程度で返答を追記してください。"
...
def main():
    ...
    while True:
        if os.path.exists(SHARED_FILE):
            ...
            # 発言回数をカウント
            academic_count = content.count("[Academicの意見]")
            
            # 上限に達していたら最終まとめて終了
            if academic_count >= MAX_ROUNDS:
